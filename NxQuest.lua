...
function CarboniteQuest:OnInitialize()
	...
	if not hideLegionEmmissaries and pLvl > 44 then emmLegion = C_QuestLog.GetBountiesForMapID(627) or {} end
					...
					if (currentStage > 0) then
						...
						for criteria = 1, numCriteria do
							...
							if criteriaInfo then
								...
								local finished = criteriaInfo.completed
...
--	end	
	...
	local func = function ()
		...
		local worldquestzones = { 947, 830, 885, 882, 1355, 1462, 1525, 1533, 1536, 1543, 1565 }	
...
-- Define reward types and faction IDs for clarity
local REWARD_TYPES = {
	AP = 10,
	GOLD = 20,
	ORDER_RESOURCES = 30,
...
-- Utility function to get faction name by ID
local function GetFactionName(factionId)
	local factionData = C_Reputation.GetFactionDataByID(factionId)
	return factionData and factionData.name or ""
end

-- Function to determine if a quest should be filtered
local function ShouldFilterQuest(questId, info, reward, faction, isBounty)
  local profile = Nx.qdb.profile.WQList
  return (
    (reward == REWARD_TYPES.AP and not profile.showap) or
    (reward == REWARD_TYPES.GOLD and not profile.showgold) or
    (reward == REWARD_TYPES.ORDER_RESOURCES and not profile.showorder) or
    (reward == REWARD_TYPES.GEAR and not profile.showgear) or
    (info.PVP and not profile.showpvp) or
    (faction == FACTION_IDS.FARONIS and not profile.showfaronis) or
    (faction == FACTION_IDS.DREAMWEAVER and not profile.showdreamweaver) or
    (faction == FACTION_IDS.HIGHMOUNTAIN and not profile.showhighmountain) or
    (faction == FACTION_IDS.LEGIONFALL and not profile.showlegionfall) or
    (faction == FACTION_IDS.NIGHTFALLEN and not profile.shownightfallen) or
    (faction == FACTION_IDS.WARDENS and not profile.showwardens) or
    (faction == FACTION_IDS.VALARJAR and not profile.showvalarjar) or
    (faction == FACTION_IDS.KIRIN_TOR and not profile.showkirintor) or
    (faction == FACTION_IDS.ARMY_OF_LIGHT and not profile.showarmyoflight) or
    (faction == FACTION_IDS.ARGUSSIAN and not profile.showargussian) or
    (not isBounty and profile.showbounty) or
    (info.mapid ~= Nx.Map:GetCurrentMapAreaID() and profile.zoneonly) or
    (not reward and not profile.showother)
  )
end

-- Function to add an item to the list
local function AddListItem(list, title, factionName, rewardString, timeStr, colString, info)
  local newWidth
